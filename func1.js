...
function changepower(){
	hero.power+=5;
	document.getElementById("power").innerHTML="攻击力~~~~ "+hero.power;
	document.getElementById("power2").innerHTML="攻击力~~~~ "+herotwo.power;
}
...
var operation=function(y,x)
{
	...
	else if(map[y][x]==11||map[y][x]==12)
	{
		...
		else
		{
			...
			heromove();
			time=setTimeout("opendoor();heromove()",600);
		}
		}
		if(playtwo==2)
		{
		...
		else
		{
			...
			map[y][x]=13;//dead monster
			heromove();
			time=setTimeout("opendoor();heromove()",500);
		}
		}
	}
	else if(map[y][x]==10)
	...
	{	
		...
		map[y][x]=20;
		heromove();
		time=setTimeout("changemoney()",800);
	}
	else if(map[y][x]==16)
	{	
		py=y;
...
$(document).ready(function () { 
	$("#canvasContainer").fadeOut(500);
	$("#textInputSpan").fadeOut(500);	
	drawmap();
	heromove();
});
